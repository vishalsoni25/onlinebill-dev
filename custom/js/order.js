...
function printOrder(orderId = null) {
	if(orderId) {		
		...
		var url = 'php_action/roughmemo.php?orderid=' + encodeURIComponent(orderId);
		console.log(url);
		...
		success:function(response) {
			...
			var tr = '<tr id="row'+count+'" class="'+arrayNumber+'">'+			  				
				...
				'<td style="padding-left:40px;">'+
					'<div class="form-group">'+
					'<input type="number"  placeholder="N.weight" max="1000000" step=".01" min="0" name="n_weight[]" id="n_weight'+count+'"   onkeyup="getTotal('+count+')" autocomplete="off" class="form-control"  />'+
					'</div>'+
				'</td>'+
				'<td style="padding-left:40px;">'+
					'<div class="form-group">'+
					'<input type="number"  placeholder="HUID" name="huid[]" id="huid'+count+'"  autocomplete="off" class="form-control"  />'+
					'</div>'+
				'</td>'+
				
				'<td style="padding-left:40px;">'+
					'<div class="form-group">'+
					'<input type="number"  placeholder="Rate" name="grate[]" id="grate'+count+'"   onkeyup="getTotal('+count+')" autocomplete="off" class="form-control"  />'+
					'</div>'+
				'</td>'+
				'<td style="padding-left:40px;">'+
					'<div class="form-group">'+
					'<input type="number"  placeholder="HallMark" name="hallmark[]" id="hallmark'+count+'"   onkeyup="getTotal('+count+')" autocomplete="off" class="form-control" />'+
					'</div>'+
				'</td>'+
				'<td style="padding-left:40px;">'+
					'<div class="form-group">'+
					'<input type="number"  placeholder="Labour/per" name="l_p_g[]" id="l_p_g'+count+'"   onkeyup="getTotal('+count+')" autocomplete="off" class="form-control" value="0" />'+
					'</div>'+
				'</td>'+
				'<td style="padding-left:40px;">'+
					'<div class="form-group">'+
					'<input type="number"  placeholder="gst" name="gst[]" id="gst'+count+'"   onkeyup="getTotal('+count+')" autocomplete="off" class="form-control" value="0" />'+
					'</div>'+
				'</td>'+
				
				'<td style="padding-left:20px;">'+
					'<input type="text" name="total[]" style="width: 90px; height:34px;" id="total'+count+'" autocomplete="off" class="form-control" disabled="true" />'+
					'<input type="hidden" name="totalValue[]" style="width: 90px; height:34px;" id="totalValue'+count+'" autocomplete="off" class="form-control" />'+
				'</td>'+
				'<td>'+
...
function getProductData(row = null) {
	if(row) {
		...
		var productId = $("#productName"+row).val();		
		var grate =  parseInt($("#grate"+row).val(), 10); 
		var n_weight = $("#n_weight"+row).val();
		var l_p_g =  parseInt($("#l_p_g"+row).val(), 10); 
		var hallmark =  parseInt($("#hallmark"+row).val(), 10); 
		var gst =  parseInt($("#gst"+row).val(), 10); 
		if(l_p_g == 0){
			var finalRate = grate;
		}
		else {
			var finalRate = grate+l_p_g;
		}
		...
		if(productId == "") {
			...
			$.ajax({
				...
				success:function(response) {
					...
					$("#hsncode"+row).val(response.rate);
					$("#hsncodeValue"+row).val(response.rate);

					$("#quantity"+row).val(1);

					var total = Number(response.rate) * 1;
					total = total.toFixed(2);
					var onSubamount = (finalRate*n_weight)+hallmark;
					var gstOnSubamount = (onSubamount * gst)/100;
					total=gstOnSubamount+onSubamount;
					$("#total"+row).val(total);
