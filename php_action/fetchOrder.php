...
$sql = "SELECT order_id, order_date, client_name, client_contact, payment_status,due FROM orders WHERE order_status = 1 ORDER BY order_date DESC";
...
if($result->num_rows > 0) { 
 ...
 while($row = $result->fetch_array()) {
 	...
 	if($row[4] == 1) { 		
 		...
 		$paymentStatus = "<label class='label label-info'>Half Payment</label>";
 	} else { 		
